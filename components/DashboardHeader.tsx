		...
		<div className="flex justify-between md:-mx-8 md:px-8 md:border-b md:pb-4 md:border-gray-700 md:bg-[#0f172a] md:-mt-8 md:pt-4 items-center mb-8">
			{Header?.key == "generate_ppt" ? (
				<div>
					<div className="flex items-center gap-4">
						...
						<div
							className="inline-flex shrink-0 items-center gap-2 px-3 py-1 rounded-full
                  bg-blue-900/30
				...
				<div>
					<h1 className="text-3xl font-bold tracking-tight w-full">
						{Header?.title}{" "}
						<span className="bg-clip-text text-transparent bg-linear-to-r from-blue-600 to-purple-600">
							{Header?.key == "dashboard" ? user?.username?.split(" ")[0] : Header?.mainTitle}
						</span>
					</h1>
			...
			<div className="flex items-center gap-4">
				{Header?.buttons?.map((button: ButtonType, index: number) => (
					<Button key={index} onClick={() => button.onclick()} className={button?.style}>
						{/* The Shimmer Effect */}
