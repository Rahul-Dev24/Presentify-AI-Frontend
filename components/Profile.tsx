...
const Profile = ({ user, logout, loading, width, height }: Props) => {
    ...
    const [backgroundColor, setBackgroundColor] = useState(localStorage.getItem('avatarColor') || `rgb(${Math.round(Math.random() * 255)}, ${Math.round(Math.random() * 255)}, ${Math.round(Math.random() * 255)})`);
