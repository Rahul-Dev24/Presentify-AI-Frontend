				...
				<div className="grid grid-cols-12 gap-6">
					...
					<Card className="col-span-12 lg:col-span-4 bg-linear-to-br from-blue-600/20 to-purple-600/20 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 text-white relative overflow-hidden group">
						...
						<div className="relative z-10 h-full flex flex-col justify-between">
							<div>
								<Badge className="bg-blue-500/20 border-blue-500/30 text-blue-300 mb-6 px-3 py-1 rounded-full">
									Pro Member
								</Badge>
								<h3 className="text-4xl font-bold tracking-tighter mb-1">
									$29<span className="text-lg font-medium opacity-40">/mo</span>
								</h3>
					...
					<Card className="col-span-12 bg-white/[0.01] border-dashed border-white/10 backdrop-blur-md p-8 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-8 transition-all hover:border-white/20">
						...
						<div className="flex gap-4">
							{[Github, Twitter, Globe, Wallet].map((Icon, i) => (
								<button
									key={i}
									className="h-12 w-12 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl flex items-center justify-center transition-all group/icon"
								>
									<Icon
										size={18}
										className="text-slate-500 group-hover/icon:text-white group-hover/icon:scale-110 transition-all"
									/>
								</button>
							))}
						</div>
