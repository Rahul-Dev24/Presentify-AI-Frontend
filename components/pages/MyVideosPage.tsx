			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{MY_PROJECTS.map((project) => (
					...
					>
						...
						<CardContent className="p-5 bg-transparent">
							<div className="flex justify-between items-start">
								...
								<DropdownMenu>
									...
									>
										<DropdownMenuItem className="gap-2 focus:bg-white/10 cursor-pointer">
											<Download size={16} /> Download .pptx
										</DropdownMenuItem>
										<DropdownMenuItem className="gap-2 focus:bg-white/10 cursor-pointer text-red-400 focus:text-red-400">
						...
						<div
							className={`h-1 w-full bg-linear-to-r ${project.status === "processing" ? "from-amber-400/50 to-orange-500/50" : "from-blue-600/50 to-purple-600/50"}`}
						/>
					</Card>
				))}
			</div>
