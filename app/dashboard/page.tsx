...
const Dashboard = () => {
				...
				buttons: [
					{
						...
						style:
							"group relative hidden md:flex items-center gap-2 px-6 py-5 bg-linear-to-r from-blue-500 to-purple-500 hover:scale-[1.02] active:scale-95 transition-all duration-300 rounded-xl border-t border-white/20 shadow-lg shadow-blue-500/20 overflow-hidden",
	...
	return (
		<div className="flex h-screen bg-[#0f172a] text-slate-100  overflow-hidden">
			...
			{isSidebarOpen && <div className="fixed inset-0 text-slate-100 z-40 md:hidden" onClick={() => toggleSidebar()} />}
      ...
      `}
			>
				...
				{/* Upgrade Card */}
				<div className="m-4 p-4 absolute w-[88%] bottom-0 rounded-2xl bg-linear-to-br from-slate-900 to-slate-800 dark:from-blue-950 dark:to-slate-900 text-white overflow-hidden group">
			...
			<main className="flex-1 overflow-y-auto">
				...
				<header className="flex scale-z-95 items-center justify-between p-4 border-b bg-gray-800 md:hidden">
					...
					<div className="flex items-center justify-end gap-4 md:hidden">
						{loading ? (
							...
							<React.Fragment>
								{isAuthenticated && <Profile user={user} logout={logout} loading={loading} />}
							</React.Fragment>
						)}
				...
				<div className="p-8 scale-z-95">
					{currentTab === "dashboard" ? (
						<>
							<DashboardHeader Header={HeaderList[0]} /> <DashboardPage />{" "}
						</>
